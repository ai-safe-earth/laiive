...
- **`main` is production, `develop` is the trunk** — full model in `CONTRIBUTING.md`. Cut
  ...
  merge `main` back into `develop` so the tag is not stranded — **locally**
  (`git checkout develop && git merge main && git push origin develop`), never as a PR with
  `main` as the head branch. The repo deletes head branches on merge, and the owner's role
  bypasses the "main is production" ruleset, so merging that PR deletes production's branch:
  PR #66 did exactly that on 2026-08-23 and it went unnoticed for two days.
